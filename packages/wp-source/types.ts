import { AsyncAction, Action, State, Derived } from "silea/types";
import Source, { EntityData } from "@sileajs/source/types";
import { Api } from "./src/libraries";

/**
 * A Silea source package for the REST API of self-hosted WordPress and
 * WordPress.com sites.
 */
interface WpSource extends Source {
	...
	actions: {
		...
		source: {
			 ...
			 *
			 * @remarks
			 * This action is not meant to be run by the user, but by the Silea
			 * framework.
			 */
	...
	libraries: {
		...
		source: Source["libraries"]["source"] & {
			...
			/**
			 * A library helper to add entities to the Silea state.
			 *
			 * @remarks
			...
			populate: (args: {
				...
				/**
				 * The subdirectory of the Silea site. When this options is passed,
