...
describe("populate", () => {
	...
	test("does not overwrite added entities", async () => {
			...
			response: mockResponse([
				{
					id: 1,
					count: 5,
					link: "https://test.sileajs.com/category/cat-1/",
					slug: "cat-1",
					taxonomy: "category",
		...
		expect(state.source.category[1].description).toBe(
			"This is the Category 1"
		);
	...
	test("removes WP API path from links", async () => {
		const { state } = initStore();
		state.source.api = "https://test.sileajs.com/subdirectory/wp-json";
