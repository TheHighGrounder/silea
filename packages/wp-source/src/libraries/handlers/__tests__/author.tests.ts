import { createStore, InitializedStore, observe } from "@sileajs/connect";
...
beforeEach(() => {
	...
	store.state.source.api = "https://test.sileajs.com/wp-json";
...
describe("author", () => {
	...
	test("is requested with a search param", async () => {
		...
		expect(
			store.state.source.data["/author/author-1/?s=findAuthor"]
				.searchQuery
		).toBe("findAuthor");
		expect(store.state.source).toMatchSnapshot();
	});
