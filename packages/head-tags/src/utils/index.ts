...
export const useSileaLinks = ({
	 ...
	 * `state.silea.url` are defined.
	 */
	if (!state.headTags.transformLinks || !state.silea.url) return headTags;
...
export const getCurrentHeadTags = ({
	...
	// Do not change links if `state.silea.url` is not defined.
	if (!state.silea || !state.silea.url) {
		console.warn(
			msg(
	...
	// Transform links.
	return useSileaLinks({ state, headTags });
};
