...
const defaultImports: Imports = {
	// "import { styled } from 'silea'" ==> "import styled from '@emotion/styled'"
	styled: "@emotion/styled",
};

// Named imports that end up as named imports.
const namedImports: Imports = {
	// "import { css } from 'silea'" ==> "import { css } from '@emotion/core'"
	css: "@emotion/core",
	// "import { Global } from 'silea'" ==> "import { Global } from '@emotion/core'"
	Global: "@emotion/core",
	// "import { keyframes } from 'silea'" ==> "import { keyframes } from '@emotion/core'"
	keyframes: "@emotion/core",
};

// Import names that should be looking for in our "silea" imports.
...
// because "silea" doesn't have any default import (ImportDefaultSpecifier).
			...
			Program(programPath) {
				programPath.traverse({
					ImportDeclaration: function (
						...
						if (path.node.source.value === "silea") {
							...
							const hasImports = memberImports
								.map(
									(memberImport) => memberImport.imported.name
								)
									...
									if (defaultImports[memberName]) {
										...
										transforms.push(
											types.importDeclaration(
												[memberImport],
												types.stringLiteral(
													namedImports[memberName]
												)
											)
										);
									} else {
										// 5. The rest of the imports are just preserved under "silea".
										transforms.push(
											types.importDeclaration(
												[memberImport],
												types.stringLiteral("silea")
											)
										);
									}
								});
