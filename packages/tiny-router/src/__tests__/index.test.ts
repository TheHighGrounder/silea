...
import { Context } from "silea/types";
import tinyRouter from "..";
import TinyRouter from "../../types";
import { SetOptions } from "@sileajs/router/types";
...
const createStore = silea.createStore;
...
describe("actions", () => {
	...
	describe("init", () => {
		...
		test("should fire `replaceState` once and add an event listener to handle popstate events", () => {
			config.state.silea.platform = "client";
			const store = createStore(config);
			...
			window.dispatchEvent(
				new PopStateEvent("popstate", {
					state: { some: "different state" },
				})
			);

			expect(store.state.router.link).toBe(link);
			expect(store.state.router.state).toEqual({
	...
	describe("beforeSSR", () => {
		test("should warn if autoFetch is enabled but there is no source pkg", () => {
			...
			get.mockReturnValue({});
			Object.defineProperty(silea, "warn", {
				value: jest.fn(),
			});
			const store = createStore(config);
			...
			expect(silea.warn).toHaveBeenCalledTimes(1);
			expect(silea.warn).toHaveBeenCalledWith(
