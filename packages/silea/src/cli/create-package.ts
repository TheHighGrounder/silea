...
export default async ({
	...
	if (!(await isSileaProjectRoot(options.projectPath))) {
		errorLogger(
			new Error(
				"You must execute this command in the root folder of a Silea project."
			)
		);
	}
	...
	if (!isThemeNameValid(options.name)) {
		errorLogger(
			new Error(
				"The name of the package is not a valid npm package name."
			)
		);
	}
