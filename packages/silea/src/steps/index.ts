...
const faviconUrl = "https://favicon.sileajs.com/";
...
export const createPackageJson = async (
	...
	// Add Silea packages to the dependencies.
	const dependencies = (
		await Promise.all(
...
// This function creates a `silea.settings` file.
export const createSileaSettings = async (
	extension: string,
	name: string,
	path: string,
	theme: string
	...
	const sileaSettings = {
		...
		state: {
			silea: {
				...
				description: "WordPress installation for Silea development",
			},
		},
		packages: [
			{
	...
	const fileTemplate = await readFile(
		resolvePath(
			__dirname,
			`../../templates/settings-${extension}-template`
		),
		{ encoding: "utf8" }
	);
	const filePath = resolvePath(path, `silea.settings.${extension}`);
	const fileData = fileTemplate.replace(/\$([\w-]+)\$/g, (_match, key) => {
		if (key === "settings") return JSON.stringify(sileaSettings, null, 2);
	});
	await writeFile(filePath, fileData);
};
...
// This function installs the Silea packages.
...
export const revertProgress = async (dirExisted: boolean, path: string) => {
	if (dirExisted) {
		...
		const removableContent = content
			.filter(
				(item) => !allowedExistingContent.includes(item.toLowerCase())
			)
