...
describe("create", () => {
	beforeEach(() => {
		...
		mockedSteps.createSileaSettings.mockReset();
	...
	test("goes through all steps", async () => {
		...
		expect(mockedSteps.createSileaSettings.mock.calls).toMatchSnapshot();
	...
	test("works correctly when `options.typescript` is false", async () => {
		...
		expect(mockedSteps.createSileaSettings).toHaveBeenCalledWith(
			"js",
			options.name,
			options.path,
			"@sileajs/mars-theme"
	...
	test("Clone the specified theme", async () => {
		const options = {
			name: "random-name",
			path: "/path/to/project",
			theme: "@sileajs/twentytwenty-theme",
		};
