...
describe("validateSettings", () => {
	test("should throw if some multi settings is missing its name", () => {
		expect(() =>
			validateSettings(mockedSettingsMissingName as any)
		).toThrow("All the settings must have a unique name.");
	});
	...
	test("should throw if some package has a repeated name", () => {
		expect(() =>
			validateSettings(mockedPackageRepeatedName as any)
		).toThrow("All the packages must have a unique name.");
	});
});
