...
export default ({
	...
	state.silea.packages.forEach((name) => {
		const variable = getVariable(name, state.silea.mode);
	...
	const debug = !!config.state?.silea?.debug;
	...
	if (process.env.NODE_ENV !== "production") config.state.silea.debug = debug;
