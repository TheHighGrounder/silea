...
/**
 * This file gets transpiled to JS anyway, but if the users's silea.settings.(js|ts)
 ...
 * silea.settings.(js|ts) file like:
 *
 * ```
 * export default {
 *   name: 'my-theme',
...
// Start Silea development environment.
export default async ({
	...
	// Get config from silea.config.js files.
	const sileaConfig = getSilea();
	const { outDir } = sileaConfig;
	...
	// Get all sites configured in silea.settings.js with their packages.
	...
	// Get config for webpack, babel and silea.
