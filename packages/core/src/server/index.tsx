...
import { getSettings } from "@sileajs/file-settings";
import { Context } from "@sileajs/types";
...
export default ({ packages }): ReturnType<Koa["callback"]> => {
	...
	// Return Silea favicon for favicon.ico.
	app.use(get("/favicon.ico", serve("./")));

	// Silea server rendering.
	app.use(async (ctx, next) => {
		...
		if (stats && hasEntryPoint({ stats, site: settings.name })) {
			...
			const crossorigin =
				moduleStats && es5Stats ? { crossorigin: "" } : {};
			silea.link = extractor.getLinkTags(crossorigin);
			...
			silea.script = `<script id="__SILEA_CONNECT_STATE__" type="application/json">${htmlescape(
		...
		ctx.body = template({ html, silea, head });
		next();
