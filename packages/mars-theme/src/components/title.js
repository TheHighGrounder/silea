...
import { Head, connect, decode } from "silea";

const Title = ({ state }) => {
	// Get data about the current URL.
	const data = state.source.get(state.router.link);
	// Set the default title.
	let title = state.silea.title;
	...
	if (data.isTaxonomy) {
		...
		title = `Author: ${decode(name)} - ${state.silea.title}`;
		...
		// 3. Render the proper title.
		title = `${cleanTitle} - ${state.silea.title}`;
	} else if (data.is404) {
		// Add titles to 404's.
		title = `404 Not Found - ${state.silea.title}`;
	}
